...
const REMEMBRANCE_ROOT = path.join(__dirname, "..", "images", "remembrance");
...
const removePostFolder = (postId) => {
  const dir = path.join(REMEMBRANCE_ROOT, String(postId));
  if (fs.existsSync(dir)) {
...
const getNextOrder = async () => {
  const last = await Remembrance.findOne().sort({ order: -1 }).select("order");
  return typeof last?.order === "number" ? last.order + 1 : 0;
};

const saveHeroImage = (postId, tempFile) => {
  const postDir = path.join(REMEMBRANCE_ROOT, String(postId));
  ensureDir(postDir);
  ...
  return `/images/remembrance/${postId}/${filename}`;
};

const resolveExistingGalleryFile = (postId, src) => {
  if (!src || typeof src !== "string") return null;
  const prefix = `/images/remembrance/${postId}/gallery/`;
  const at = src.indexOf(prefix);
  if (at === -1) return null;
  const filename = path.basename(src.slice(at + prefix.length));
  if (!filename || filename.includes("..")) return null;
  const full = path.join(REMEMBRANCE_ROOT, String(postId), "gallery", filename);
...
const rebuildGallery = (postId, galleryOrder, galleryFiles = []) => {
  ...
  try {
    galleryOrder.forEach((item, index) => {
      ...
      result.push({
        src: `/images/remembrance/${postId}/gallery/${filename}`,
        caption,
      });
    });
...
exports.getBySlug = async (req, res) => {
  try {
    ...
    if (index === -1) {
      return res.status(404).json({ message: "Remembrance post not found" });
    }
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const existing = await Remembrance.findOne({ slug: fields.slug });
    if (existing) {
      cleanupTemp(hero);
      cleanupTemps(galleryFiles);
    ...
    const post = await Remembrance.create({
      ...fields,
      order,
      image: "",
      gallery: [],
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const post = await Remembrance.findById(req.params.id);
    if (!post) {
      cleanupTemp(hero);
      cleanupTemps(galleryFiles);
      return res.status(404).json({ message: "Remembrance post not found" });
    ...
    if (fields.slug !== post.slug) {
      const taken = await Remembrance.findOne({
        slug: fields.slug,
        _id: { $ne: post._id },
      });
      if (taken) {
...
exports.reorder = async (req, res) => {
  try {
    ...
    const allPosts = await Remembrance.find().select("_id");
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "orderedIds must include every remembrance post id exactly once",
      });
    }

    await Promise.all(
      orderedIds.map((id, index) =>
        Remembrance.findByIdAndUpdate(id, { order: index })
      )
    );

    const posts = await Remembrance.find().sort({ order: 1 });
