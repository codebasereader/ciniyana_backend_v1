...
const OTC_ROOT = path.join(__dirname, "..", "images", "off-the-camera");
const LAYOUTS = ["overlap", "banner", "poster", "landscape"];
...
const removePostFolder = (postId) => {
  const dir = path.join(OTC_ROOT, String(postId));
  if (fs.existsSync(dir)) {
...
const mapBodyFields = (body) => {
  const layout = String(body.layout || "overlap").trim();
  ...
  return {
    ...
    layout: LAYOUTS.includes(layout) ? layout : "overlap",
...
const getNextOrder = async () => {
  const last = await OffTheCamera.findOne().sort({ order: -1 }).select("order");
  return typeof last?.order === "number" ? last.order + 1 : 0;
};

const saveNamedImage = (postId, tempFile, basename) => {
  const postDir = path.join(OTC_ROOT, String(postId));
  ensureDir(postDir);
  ...
  return `/images/off-the-camera/${postId}/${filename}`;
};
...
const resolveExistingGalleryFile = (postId, src) => {
  ...
  const prefix = `/images/off-the-camera/${postId}/gallery/`;
  const at = src.indexOf(prefix);
  if (at === -1) return null;
  const filename = path.basename(src.slice(at + prefix.length));
  if (!filename || filename.includes("..")) return null;
  const full = path.join(OTC_ROOT, String(postId), "gallery", filename);
...
const rebuildGallery = (postId, galleryOrder, galleryFiles = []) => {
  ...
  try {
    galleryOrder.forEach((item, index) => {
      ...
      result.push({
        src: `/images/off-the-camera/${postId}/gallery/${filename}`,
        caption,
      });
    });
...
exports.list = async (req, res) => {
  try {
    const docs = await OffTheCamera.find().sort({ order: 1 });
    return res.status(200).json({ posts: docs.map(toPost) });
  } catch (error) {
    return res.status(500).json({
      message: "Failed to list off the camera posts",
      error: error.message,
    });
  }
};
...
exports.getBySlug = async (req, res) => {
  try {
    ...
    if (index === -1) {
      return res.status(404).json({ message: "Off the Camera post not found" });
    }
    ...
    return res.status(500).json({
      message: "Failed to get off the camera post",
      error: error.message,
    });
  }
};
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const existing = await OffTheCamera.findOne({ slug: fields.slug });
    if (existing) {
      cleanupTemp(hero);
      cleanupTemp(profileImage);
      cleanupTemps(galleryFiles);
    ...
    const post = await OffTheCamera.create({
      slug: fields.slug,
      date: fields.date,
      title: fields.title,
    ...
    try {
      ensureDir(path.join(OTC_ROOT, String(post._id)));
      ...
      await OffTheCamera.findByIdAndDelete(post._id);
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const post = await OffTheCamera.findById(req.params.id);
    if (!post) {
      cleanupTemp(hero);
      cleanupTemp(profileImage);
      cleanupTemps(galleryFiles);
    ...
    if (fields.slug !== post.slug) {
      const taken = await OffTheCamera.findOne({
        slug: fields.slug,
        _id: { $ne: post._id },
      });
      if (taken) {
    ...
    return res.status(500).json({
      message: "Failed to update off the camera post",
      error: error.message,
    });
  }
};
...
exports.remove = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Failed to delete off the camera post",
      error: error.message,
    });
  }
};
...
exports.reorder = async (req, res) => {
  try {
    ...
    const allPosts = await OffTheCamera.find().select("_id");
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "orderedIds must include every off the camera post id exactly once",
      });
    }

    await Promise.all(
      orderedIds.map((id, index) =>
        OffTheCamera.findByIdAndUpdate(id, { order: index })
      )
    );

    const posts = await OffTheCamera.find().sort({ order: 1 });
    ...
    return res.status(500).json({
      message: "Failed to reorder off the camera posts",
      error: error.message,
    });
  }
};
