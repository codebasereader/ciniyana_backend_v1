...
const INFO_SPECIAL_ROOT = path.join(__dirname, "..", "images", "info-special");
...
const toPost = (doc) => {
  ...
  return {
    ...
    galleryMode: obj.galleryMode || "carousel",
...
const removePostFolder = (postId) => {
  const dir = path.join(INFO_SPECIAL_ROOT, String(postId));
  if (fs.existsSync(dir)) {
...
const mapBodyFields = (body) => {
  ...
  const galleryMode = String(body.galleryMode || "carousel").trim();
  ...
  return {
    ...
    galleryMode: GALLERY_MODES.includes(galleryMode)
      ? galleryMode
      : "carousel",
    profileName: {
...
const getNextOrder = async () => {
  const last = await InfoSpecial.findOne().sort({ order: -1 }).select("order");
  return typeof last?.order === "number" ? last.order + 1 : 0;
};

const saveNamedImage = (postId, tempFile, basename) => {
  const postDir = path.join(INFO_SPECIAL_ROOT, String(postId));
  ensureDir(postDir);
  ...
  return `/images/info-special/${postId}/${filename}`;
};
...
const resolveExistingGalleryFile = (postId, src) => {
  ...
  const prefix = `/images/info-special/${postId}/gallery/`;
...
const rebuildGallery = (postId, galleryOrder, galleryFiles = []) => {
  ...
  try {
    galleryOrder.forEach((item, index) => {
      ...
      result.push({
        src: `/images/info-special/${postId}/gallery/${filename}`,
        caption,
      });
    });
...
exports.getBySlug = async (req, res) => {
  try {
    ...
    if (index === -1) {
      return res.status(404).json({ message: "Info Special post not found" });
    }
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const existing = await InfoSpecial.findOne({ slug: fields.slug });
    if (existing) {
      cleanupTemp(hero);
      cleanupTemp(profileImage);
      cleanupTemps(galleryFiles);
    ...
    const post = await InfoSpecial.create({
      slug: fields.slug,
      date: fields.date,
      title: fields.title,
      body: fields.body,
    ...
    try {
      ensureDir(path.join(INFO_SPECIAL_ROOT, String(post._id)));
      ...
      await InfoSpecial.findByIdAndDelete(post._id);
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const post = await InfoSpecial.findById(req.params.id);
    if (!post) {
      cleanupTemp(hero);
      cleanupTemp(profileImage);
      cleanupTemps(galleryFiles);
    ...
    if (fields.slug !== post.slug) {
      const taken = await InfoSpecial.findOne({
        slug: fields.slug,
        _id: { $ne: post._id },
      });
      if (taken) {
...
exports.reorder = async (req, res) => {
  try {
    ...
    const allPosts = await InfoSpecial.find().select("_id");
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "orderedIds must include every info special post id exactly once",
      });
    }

    await Promise.all(
      orderedIds.map((id, index) =>
        InfoSpecial.findByIdAndUpdate(id, { order: index })
      )
    );

    const posts = await InfoSpecial.find().sort({ order: 1 });
